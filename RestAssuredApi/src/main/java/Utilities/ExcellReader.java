...
public class ExcellReader {
	...
	public ArrayList<String> getDataFromExcellSheet(String sheetName, String title, String TestCaseName) throws IOException {
		ArrayList<String> al = new ArrayList<String>();
		FileInputStream file = new FileInputStream("C:\\WebApplication\\API Automation\\APITesting\\Resources\\demoData.xlsx");
		...
		for(int i=0; i<sheets; i++) {
			if(workbook.getSheetName(i).equalsIgnoreCase(sheetName)) {
				...
				while(rows.hasNext()) {
					...
					//After we grab purchase test case row = pull all the data of that row and feed into test
